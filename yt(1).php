...
$admin = $admins[0];
	$ch = $sting['sting']['ch1'];
$ok = bots('getChatMember',['chat_id'=>$ch,'user_id'=>$mei]);
if($ch != null and $ok->ok == "true" and $ok->result->status != "left"){
if(preg_match("/(-100)(.)/", $ch) and !preg_match("/(.)(-100)(.)/", $ch)){
	$link = bots("getchat",['chat_id'=>$ch])->result->invite_link;
	if($link != null){
		$link = $link;
$link2 = $link;
		}else{
			$link = bots("exportChatInviteLink",['chat_id'=>$ch])->result;
$link2 = $link;
			}
...
$link2 = "https://t.me/$ch3";
		...
		$ok = bots('getChat',['chat_id'=>$ch]);
		$status = bots('getChatMember',['chat_id'=>$ch,'user_id'=>$from_id])->result->status;
            ...
            unset($sting['sting']['ch5']);
					...
					if($sting['sting']['ch1'] == null){
						$ch1 = "قناة 1 🔱 لا يوجد 😴";
							...
							$ch3 = bots('getchat',['chat_id'=>$sting['sting']['ch1']]);
							if($ch3->ok == true){
						...
						$ch2 = "قناة 2 🔱 لا يوجد 🌚";
							...
							$ch = bots('getchat',['chat_id'=>$sting['sting']['ch2']]);
							if($ch->ok == true){
								$ch2 = $ch->result->title;
								}else{
									$ch2 = "قناة 2 🔱 لا يوجد 🌚";
						...
						$ch3 = "قناة 3 🔱 لا يوجد 🌚";
							...
							$ch = bots('getchat',['chat_id'=>$sting['sting']['ch3']]);
							if($ch->ok == true){
								$ch3 = $ch->result->title;
								}else{
									$ch3 = "قناة 3 🔱 لا يوجد 🌚";
						...
						$ch4 = "قناة 4 🔱 لا يوجد 🌚";
							...
							$ch = bots('getchat',['chat_id'=>$sting['sting']['ch4']]);
							if($ch->ok == true){
								$ch4 = $ch->result->title;
								}else{
									$ch4 = "قناة 4 🔱 لا يوجد 🌚";
						...
						$ch5 = "قناة 5 🔱 لا يوجد 🌚";
							...
							$ch = bots('getchat',['chat_id'=>$sting['sting']['ch5']]);
							if($ch->ok == true){
								$ch5 = $ch->result->title;
								}else{
									$ch5 = "قناة 5 🔱 لا يوجد 🌚";
...
if($update->message->forward_from_chat){
	...
	if($data == "admins"){
		...
		$names = bots("getchat",["chat_id"=>$admins])->result->first_name;
		if($names != null){
...
$namer = bots('getchat',['chat_id'=>$text])->result->first_name;
if($namer){
bots('sendmessage',[
	'chat_id'=>$chat_id, 
	'text'=>"
...
if($chat_id == $admin and $sting['sting']['sting'] == 'file'){
    ...
    $file = "http://3.8.138.146:443/file/bot".API_KEY."/".bots('getfile',['file_id'=>$message->document->file_id])->result->file_path;
...
$exi = explode('#',$data);
if($exi[0]=="vid"){
bots('editMessageReplyMarkup',[
'chat_id'=>$update->callback_query->message->chat->id,
'message_id'=>$update->callback_query->message->message_id,
'reply_markup'=>null,
]);
$idyt = $exi[1];
$mid2 = bots('sendmessage',['chat_id'=>$chat_id2,'text'=>"جاري التحميل..."])->result->message_id;
